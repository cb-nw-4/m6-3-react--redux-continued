...
const ArtistRoute = ()=>{
    ...
    const {currentArtist , status, error } = useSelector((state)=>state.artists);
    ...
    return(
        ...
        {status === 'idle' && 
            <ArtistWrapper>
                ...
                <Number>{formatNum(currentArtist.profile.followers.total)} <Span>followers</Span></Number>
                <Tag>tags</Tag>
                <GenresWrapper>
